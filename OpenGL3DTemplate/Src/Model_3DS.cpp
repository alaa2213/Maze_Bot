...
}void Model_3DS::ComputeBounds() {
	min_x = min_y = min_z = FLT_MAX;
	max_x = max_y = max_z = FLT_MIN;

	for (int i = 0; i < numObjects; i++) {
		for (int j = 0; j < Objects[i].numVerts; j++) {
			float vx = Objects[i].Vertexes[j * 3];
			float vy = Objects[i].Vertexes[j * 3 + 1];
			float vz = Objects[i].Vertexes[j * 3 + 2];
